...
def _bucket_from_text(text: str, num_blocks: int) -> int:
    key = text.encode("utf-8", errors="ignore")
...
def _dense_features(text: str, proof: str, input_dim: int) -> List[float]:
    ...
    ratios = [
        ...
        math.log1p(tok_count) / 10.0,
        uniq / tok_count,
        len(proof) / max(chars, 1),
    ]

    raw = ratios + [h / 5.0 for h in key_hits] + [p / max(chars, 1) for p in punct]

    vec = np.zeros((input_dim,), dtype=np.float32)
    for i in range(min(len(raw), input_dim)):
        vec[i] = float(raw[i])

    # Add deterministic hashed tail features.
...
def prepare_minif2f_rows(rows: List[Dict], input_dim: int, num_blocks: int) -> List[Dict]:
    ...
    for row in rows:
        ...
        if "label" in row:
            ...
            # Heuristic label for MVP: rows with non-empty proof are treated as positive.
            label = 1.0 if proof.strip() else 0.0
...
def split_rows(rows: List[Dict], train_ratio: float = 0.8, seed: int = 42) -> Tuple[List[Dict], List[Dict]]:
    rng = np.random.default_rng(seed)
